		     ...
		     const std::vector<float>& targets, float learning_rate)
{
	...
	std::vector<float> outputs = MLP(inputs, weights);
	...
	for (int i = 0; i < OUTPUTS; ++i) {
		...
		output_errors.push_back(
		    error * DERIVATIVE_ACTIVATION_FUNCTION(outputs[i]));
	}
	...
	for (int i = 0; i < weights[HIDDEN_LAYERS].size(); ++i) {
		for (int j = 0; j < weights[HIDDEN_LAYERS][i].size(); ++j) {
			weights[HIDDEN_LAYERS][i][j] +=
			    learning_rate * output_errors[i] *
			    layers[HIDDEN_LAYERS][i].value;
		}
		weights[HIDDEN_LAYERS][i].bias +=
		    learning_rate * output_errors[i];
	}
	...
	for (int i = HIDDEN_LAYERS - 1; i >= 0; --i) {
		...
		for (int j = 0; j < weights[i].size(); ++j) {
			...
			for (int k = 0; k < weights[i + 1].size(); ++k) {
				error +=
				    output_errors[k] * weights[i + 1][k][j];
			}
			hidden_errors.push_back(
			    error *
			    DERIVATIVE_ACTIVATION_FUNCTION(layers[i][j].value));
		}
...
int main()
{
	...
	init_vec(weights, HIDDEN_LAYERS + 1, NEURON_PER_LAYER,
		 NEURON_PER_LAYER);
